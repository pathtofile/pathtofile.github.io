...
For our example, lets say these fds are `3` for the read end, and `4` for the write end.
...
When apple is created using `clone`, it isn't running the `apple` binary straight away - it's still `bash`, just a clones version of it.
